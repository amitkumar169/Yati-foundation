...
<h1 style="text-align:center;color:white;text-shadow:2px 2px 2px white;">Donate Management</h1>
...
if($conn==true)	
{
 ...
 if(mysqli_num_rows($x))
 {
	...
	{
		$a++;
	echo "<tr><td>$a</td><td>".$row["name"]."</td><td>".$row["email"]."</td><td>".$row["mob"]."</td>
	<td>".$row["amount"]."</td><td>".$row["pdonation"]."</td><td>".$row["address"]."</td><td>".$row["ddt"]."</td>
	<td><a href='../codes/deldon.php/?v=".$row["email"]."'><span class='fa fa-trash'></span></a></td></tr>";	
	}
 }
 else
 {
